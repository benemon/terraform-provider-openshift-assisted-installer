...
func TestSupportLevelsDataSource_Read(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/api/assisted-install/v2/support-levels/features":
			...
			_ = json.NewEncoder(w).Encode(mockFeatures)

		case "/api/assisted-install/v2/support-levels/architectures":
			// Verify query parameters
			assert.Equal(t, "4.14.0", r.URL.Query().Get("openshift_version"))

			w.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(w).Encode(mockArchitectures)
...
func TestSupportLevelsDataSource_ReadError_Architectures(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/assisted-install/v2/support-levels/features":
			w.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(w).Encode(models.SupportedFeatures{})
		case "/api/assisted-install/v2/support-levels/architectures":
			w.WriteHeader(http.StatusBadRequest)
			_, _ = w.Write([]byte(`{"error": "invalid version"}`))
		}
